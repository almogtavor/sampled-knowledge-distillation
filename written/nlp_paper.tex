...
AT-KD baseline. We also include Adaptive Teaching KD (AT-KD; \citealt{zhong2024atkd}) as a strong routing baseline.
Per mini-batch, AT-KD ranks tokens by the teacher's uncertainty coefficient \(\mathrm{UnC}=p_{t\setminus g}=1-p_{tg}\) and splits them into easy \(\mathcal{D}_e\) and hard \(\mathcal{D}_h\).
With binary top-vs-rest probabilities \(p_b=[p_{tg},p_{t\setminus g}]\) and non-target distribution \(\hat p\), the objective is
\(\;\;\mathcal{L}^{e}_{\mathrm{KL}}=\sum_{t\in\mathcal{D}_e}\!\mathrm{KL}(\hat p^t\|\hat q^t),\;\;
\mathcal{L}^{h}_{\mathrm{KL}}=\sum_{t\in\mathcal{D}_h}\!\big[\mathrm{KL}(p^t_b\|q^t_b)+\mathrm{KL}(\hat p^t\|\hat q^t)\big],\;\;
\mathcal{L}^{\mathrm{all}}_{\mathrm{KL}}=\lambda\,\mathcal{L}^{e}_{\mathrm{KL}}+(1-\lambda)\,\mathcal{L}^{h}_{\mathrm{KL}}\) with \(\lambda{=}0.2\).
We report two variants: (i) online-teacher AT-KD (no cache) and (ii) cached AT-KD that reuses our RS-KD over classes while keeping AT-KD's token routing.

Relation to our setup. Our entropy \(H(p_t)\) and AT-KD's \(\mathrm{UnC}=1-p_{tg}\) both quantify token-level uncertainty (they are often monotone when the distribution is peaked), while AT-KD's DKD term (\(\mathrm{KL}(\hat p\|\hat q)\)) corresponds to the "dark knowledge" over non-target classes that we also model; the target/binary term (TKD) aligns with our standard CE/target supervision.
...
\begin{table*}[t]
	...
	\begin{tabular}{lccccccc}
		\toprule
		Method                                     & Arc-E & GSM8K (\%) $\uparrow$ & HellaSwag (\%) $\uparrow$ & PIQA (\%) $\uparrow$ & \multicolumn{2}{c}{LAMBADA OAI} & avg.                                    \\
		\cmidrule(lr){5-6}
		                                           &		 &                       &                           &                      & acc. (\%) $\uparrow$        & perplexity $\downarrow$                 \\
		Full KD (all tokens, full softmax,CE=0.1)  & 63.8    & 47.3                  & 37.2                      & 67.7                 & 41.1                        & 27.9                    & 52.4          \\
		Token-selective KD (entropy top-15\%)      & 64.9    & 42.3                  & 36.1                      & 67.4                 & 34.1                        & 44.9                    & 50.6          \\
		Token-selective KD (ent top-20\%,CE=0.0)   & 63.5    & 43.5                  & 35.7                      & 66.3                 & 20.3                        & 135.8                   & 46.5          \\
		Token-selective KD (ent top-20\%,CE=0.1)   & 65.2    & 45.2                  & 36.2                      & 67.6                 & 33.8                        & 45.2                    & 50.7          \\
		Token-selective KD (ent top-20\%,CE=0.3)   & 65.8    & 42.7                  & 36.8                      & 67.6                 & 40.7                        & 26.9                    & 52.8          \\
		Token-selective KD (entropy top-25\%)      & ----    & ----                  & ----                      & ----                 & ----                        & ----                    & ----          \\
		Token-selective KD (entropy top-30\%)      & ----    & ----                  & ----                      & ----                 & ----                        & ----                    & ----          \\
		...
		AT-KD (online teacher, full tokens)      & ----    & ----                  & ----                      & ----                 & ----                        & ----                    & ----          \\
		AT-KD (offline cache, RS-KD coupling)     & ----    & ----                  & ----                      & ----                 & ----                        & ----                    & ----          \\
		\midrule
...
\begin{figure}[t]
\centering
\resizebox{\columnwidth}{!}{\input{../graphs/fig_pareto_acc_vs_ppl.pgf}}
...
% In continual KD, token-level cross-entropy is used to quantify when and where to distill \citep{zhang2023continualkd}.
