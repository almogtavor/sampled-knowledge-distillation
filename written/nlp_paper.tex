...
\paragraph{Position-selection policies.}
...
\begin{enumerate}
  ...
  \item \textbf{Positional RS-KD.} Instead of picking the highest-entropy positions deterministically, we sample unique positions according to a probability distribution $q_{\text{pos}}(t)\propto \big(\text{base}_t\big)^{\alpha}$. To avoid bias, each selected position is reweighted as
	\[
	w_t \propto  \tfrac{1}{\max(q_{\text{pos}}(t),q_{\min})}, \sum_{t\in S}w_t=1,
	\]
...
\paragraph{Loss computation (common to all policies).}
...
T^2 \cdot
\begin{cases}
\sum_{t\in S} w_t\, \mathrm{KD}_t & \text{(RS-KD with Hájek weights)}\\[2pt]
\operatorname{mean}_{t\in S}\!\big[\mathrm{KD}_t\big] & \text{(top-$k$, bucket, bandit, random)}
\end{cases}
\;+\;
