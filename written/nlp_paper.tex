...
\label{sec:entropy}
We estimate token uncertainty with a \emph{Top-$m$-plus-tail} construction. Let $u\in\mathbb{R}^V$ be teacher logits and
$
	p_i=\frac{e^{u_i}}{Z},\quad Z=\sum_{j=1}^V e^{u_j}.
$
Order indices so $u_{(1)}\ge\cdots\ge u_{(m)}$ are the top-$m$ logits. Define the top-$m$ log-partition $Z_m=\log\!\sum_{j\le m} e^{u_{(j)}}$ and an upper bound on the full log-partition by
\begin{align*}
...
\begin{table}[h]
	...
	\begin{tabular}{lcccccc}
		...
		20  & 0.976                                          & 0.956                                       & 0.957 & 0.977   & \textbf{0.995} & \textbf{0.995} \\
		\bottomrule
	\end{tabular}
...
= -P_T\log\!\frac{P_T}{V-m}
\;-\; P_T \cdot \frac{1}{s}\sum_{i=1}^s \Big(\log p_{v_i} - \log u\Big).
\]
