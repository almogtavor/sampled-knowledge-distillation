...
$
denote the softmax probabilities and partition function.

\noindent\emph{Units.} We compute entropy in nats (natural logarithm). When quantizing to 8-bit, we normalize by $\ln V$. Order indices so that $u_{(1)} \geq \cdots \geq u_{(m)}$ are the top-$m$ logits. Define the top-$m$ log-partition and an upper bound on the full log-partition by
\begin{align*}
