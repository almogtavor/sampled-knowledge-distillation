...
We have evaluated the GLS policy with a queue size of $30K$ tokens, as reported to be optimal by the experiments of \citet{wang2021selectivekd}.
That said, \citep{wang2021selectivekd} focused on CE-based selection, and we found that GLS was a bit less successful for entropy-based selection than batch top-k, even though the results were fairly close and might change when scaled.
...
\begin{table*}[t]
	...
	\begin{tabular}{lcccccc}
		...
		Token-selective KD (entropy top-25\%)               & 38.4                  & 37.0                      & 67.1                 & \textbf{42.2}           & \textbf{23.4} & \textbf{48.8} \\
		...
		Token-selective KD (entropy top-25\%, GLS)          & ----                  & 36.9                      & 67.1                 & ----           & ---- & ---- \\
		\midrule
		...
		\midrule
		% Sampled KD (top-75\%), softmax-elim      			& 27.3                  & 31.2                      & 59.8                 & 24.5                  	 & 2029 & 38.5 \\
		% Sampled KD (pos-rs-kd-25\%), softmax-elim			& 23.7                  & 31.4                      & 60.6                 & 23.1                  	 & 2336 & 38.4 \\

		% \quad Top $k=75\%$ positions                        & 29.1                  & 34.5                      & 64.5                 & 38.8                  &---   & \\
		% \quad Top $k=25\%$ pos, GLS (global queue)          & 26.1                  & 32.1                      & 62.6                 & 35.2                  &---   & \\
		% \quad Top $k=25\%$ pos, Score based Selection       & 29.3                  & 34.7                      & 64.4                 & 39.1                  &---   & \\
		...
		\midrule
		No KD baseline (untrained Qwen3-0.6B)               & \textbf{47.6}         & \textbf{37.5}             & \textbf{67.6}        & 40.4                    & 24.72 & 48.2\\
		\bottomrule
	\end{tabular}
...
We attribute the remaining gap primarily to the limited training budgets (only 4M FineWeb-Edu tokens) and predominantly zero-shot evaluation, together with scale and quantization effects.
Another reason for the gap could be due to the fact we have trained with max\_seq\_len=256, and filtered all documents that were longer than that (to avoid perplexity issues).
This is shorter than the average length of the FineWeb-Edu dataset (which is $\approx750$), and that may have caused bias in the training data.
Fineweb is also a mixture of many domains, and not a good match for training a math reasoning model, which was one of our evaluation tasks that the model's performance consitently degraded at (GSM8K).
...
From a computational point of view, applying token-selective KD that removes KLD steps, but keeping CE on all tokens (for stability), loses the main efficiency gain of the time steps selection idea.
