...
\paragraph{Selecting where to supervise.} Beyond “how” to form the target, several works decide “where” to apply supervision. In NMT, \citet{wang2021selectivekd} select \emph{words} by cross-entropy using batch-level and global (FIFO) queues (GLS). For post-training, token-level uncertainty-aware objectives select \emph{tokens} by loss/entropy and add self-distillation to prevent OOD overfitting \citep{liu2025tokenlevel}. In reasoning, KPOD learns keypoint weights and a progressive schedule within chain-of-thought rationales \citep{feng2024kpod}. Data-centric selection via Selective Reflection Distillation curates training instances with a curriculum \citep{liu2025srd}. Our work differs by coupling \emph{token-level} selection with \emph{unbiased} logit sampling at selected positions, yielding efficiency without the calibration drawbacks of deterministic truncation.
