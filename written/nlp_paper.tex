...
We train our models on FineWeb-Edu with 1M and 4M tokens and evaluate them zero-shot on three diverse benchmarks: \textbf{HellaSwag} \citep{zellers2019hellaswag} (commonsense inference), \textbf{GSM8K} \citep{cobbe2021gsm8k} (grade-school math word problems), \textbf{PIQA} \citep{bisk2020piqa} (physical commonsense), and \textbf{LAMBADA} \citep{paperno2016lambada} (broad-context word prediction).
...
\begin{table*}[t]
	...
	\begin{tabular}{lcccc}
		...
		\midrule
		Full KD (all tokens, full softmax)                  & 34.6 & 34.6 & 65.1  & \textbf{40.6} \\
		Token-selective KD (entropy top-30\%)			     & 26.8  & 53.0 & ---  & ---  \\
		Token-selective KD (bucket of 5\%-30\%)            & ---  & 30.0 & ---  & ---  \\
		Token-selective KD (random 25\%)                   & ---  & 26.0 & ---  & ---  \\
		\textbf{SampledKD} (top $k=30\%$ positions + RS-KD) & 26.0 & 30.0 & 32.0 & 34.0 \\
		\quad Top $k=75\%$ positions                        & ---  & 40.0 & ---  & ---  \\
		\quad Top $k=30\%$ pos, GLS (global queue)          & ---  & 32.0 & ---  & ---  \\
		\quad Top $k=30\%$ pos, Score based Selection       & ---  & 30.0 & ---  & ---  \\
		\quad Positional RS-KD $k=30\%$ (sampled)          & ---  & 24.0 & ---  & ---  \\
		\quad $k=100\%$                                     & ---  & 26.0 & ---  & ---  \\
		\quad $k=30\%$, distilled on GSM8K train split      & 49.2 & ---  & ---  & ---  \\
		\quad LinUCB model as selector                      & ---  & 15.0 & ---  & 20.0 \\
		\midrule
		No KD baseline (untrained Qwen3-0.6B)               & 47.6 & 37.5 & 67.6 & 40.4 \\
		\bottomrule
	\end{tabular}
